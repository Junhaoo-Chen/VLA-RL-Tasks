...
@register_env("CalculatedCleaning-v1", max_episode_steps=100)
class CalculatedCleaning(BaseEnv):
    """
    Task: Use nearby objects to throw all the unreachable objects out.
    """
    ...
    def evaluate(self):
        ...
        with torch.device(self.device):
            ...
            success = torch.all(whether_object_not_on_table, dim=0)
            num_throwm = torch.sum(whether_object_not_on_table, dim=0)  # Shape: [n_batch]

        return {"success": success,
                "num_throwm": num_throwm,
                # "throwable_obj_pos": throwable_obj_pos,
                "obj_pos": obj_pos,
                # "whether_object_not_on_table": whether_object_not_on_table
                }
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        # throwable_obj_pos = info['throwable_obj_pos']
        obj_pos = info['obj_pos']
        # whether_object_on_table = info['whether_object_not_on_table']
        ...
        self.max_reward = (max_dist_reward + max_progress_reward)*2 

        # success reward
        total_reward = distances_reward + progress_reward
